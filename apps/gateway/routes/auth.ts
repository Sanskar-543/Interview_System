...
const handleSignupInit = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/create-password', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const token = jwt.sign(
      { id: updatedUser.id, email: updatedUser.email, plan: updatedUser.plan },
      env.JWT_SECRET,
      { expiresIn: '7d' },
    );
