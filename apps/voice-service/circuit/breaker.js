...
export class CircuitBreaker {
    ...
    async getState() {
        const state = await this.redis.get(REDIS_STATE_KEY);
        return state || 'CLOSED';
    }
    async recordFailure() {
     ...
     */
    async runCompletionWithFallback(messages, primaryCompletionsCall, sessionId, userId) {
        const state = await this.getState();
    ...
    async executeLevel2(messages, primaryCompletionsCall, sessionId, userId) {
    ...
    async executeLevel3(sessionId, userId) {
    ...
    async triggerCreditRefund(sessionId, userId, reason) {
        try {
