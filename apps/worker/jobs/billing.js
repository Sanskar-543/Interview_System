...
export async function processCreditRefundJob(data) {
    ...
    try {
        ...
        if (session.status !== 'failed') {
            logger.warn({ sessionId, status: session.status }, 'Worker Billing Warning: Session status is not failed. Refund request rejected.');
            return;
        }
