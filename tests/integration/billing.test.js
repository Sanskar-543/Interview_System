...
import { db, users, sessions } from '../../packages/db/index.js';
import { eq, sql } from 'drizzle-orm';
import { env } from '../../packages/shared/config/env.js';
import { billingRouter, billingWebhookHandler } from '../../apps/gateway/routes/billing.js';
import { processCreditRefundJob } from '../../apps/worker/jobs/billing.js';
...
test('Billing & Webhook Integration Suite', async (t) => {
  ...
  await t.test('1. Gated Sandbox: missing Razorpay key in production throws BILLING_MISCONFIGURED', async () => {
    ...
    env.RAZORPAY_KEY_ID = originalRazorpayKey;
...
test('Background Billing Worker Credit Refund Processing', async (t) => {
  ...
  let selectCall = 0;
  let updateValues = null;
  let mockedSessionState = null;
  let mockedUserState = null;
