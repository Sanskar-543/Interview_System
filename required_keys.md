...
|----------------------|-------------------|----------------------|
| **`CORS_ORIGIN`** | Sets allowed origins on the API Gateway. | Optional. The URL of the Vite + React frontend (e.g., `https://my-app.vercel.app` or `http://localhost:3000`). |
...
Loaded on the client-side Vite + React application.

| Environment Variable | Purpose | Default / Example |
|----------------------|---------|-------------------|
| **`VITE_API_URL`** | Vite API URL pointing to the Gateway server. | `http://localhost:5000` |
| **`VITE_WS_URL`** | Vite WebSocket URL pointing to the Gateway server. | `ws://localhost:5000` |
| **`VITE_GATEWAY_URL`** | Vite Gateway URL for auxiliary parsing endpoints. | `http://localhost:5000` |
...
# Vite Client-Side Configuration
VITE_API_URL=https://your-gateway-app.railway.app
VITE_WS_URL=wss://your-gateway-app.railway.app
VITE_GATEWAY_URL=https://your-gateway-app.railway.app
```
